...
import RosterGrid from "../[rosterId]/components/RosterGrid";
import CellModal from "../[rosterId]/components/CellModal";
...
function RosterEditInner() {
  ...
  const load = useCallback(async (tk: string) => {
    if (!rosterId) { router.replace("/dashboard"); return; }
  ...
  function navigateWeek(dir: -1 | 1) {
    ...
    found ? router.push(`/roster/edit?id=${found.id}`) : showToast("No roster for that week");
  }
